# Migration Backup - Discourse Modernization

**Date**: 2025-11-04
**Purpose**: Backup of original head_tag.html before modernizing to new Discourse standards

## What Changed

Migrated from deprecated `<script type="text/discourse-plugin">` tags to modern `apiInitializer` pattern following [Discourse modernization guidelines](https://meta.discourse.org/t/modernizing-inline-script-tags-for-templates-js-api/366482).

## Original head_tag.html Content

The original file contained 4 script blocks:
1. Dark mode detection (lines 1-48) - now in `dark-mode-detector.js`
2. Remove custom lists (lines 50-104) - was duplicate of existing `remove-custom-lists.js`
3. Resize category icons (lines 106-206) - was duplicate of existing `category-icon-resize.js`
4. Hide product icons (lines 208-290) - was duplicate of existing `hide-product-icons.js`
...
### 1. Created New File: `javascripts/discourse/initializers/dark-mode-detector.js`
- Migrated dark mode detection from inline script to modern initializer
- Uses `apiInitializer("1.8.0")` pattern
- All functionality preserved

### 2. Modified File: `common/head_tag.html`
- Removed all 4 script blocks (1 regular + 3 deprecated)
- File is now empty
- All functionality moved to proper initializers

### 3. Existing Modern Initializers (No Changes Needed)
These files were already using modern patterns:
- `javascripts/discourse/initializers/remove-custom-lists.js`
- `javascripts/discourse/initializers/category-icon-resize.js`
- `javascripts/discourse/initializers/hide-product-icons.js`

## Restoration Instructions

If you need to restore the original functionality:

1. Replace `common/head_tag.html` with the content above
2. Delete `javascripts/discourse/initializers/dark-mode-detector.js`

Or restore from git:
```bash
git checkout HEAD~1 common/head_tag.html
git rm javascripts/discourse/initializers/dark-mode-detector.js
```

## Testing Checklist

- [ ] Dark mode detection works correctly
- [ ] Dark mode toggles apply `.discourse-dark-theme` class
- [ ] Custom list items are removed from navigation
- [ ] Category icons resize correctly on homepage
- [ ] Product category icons are hidden appropriately
- [ ] No console errors or deprecation warnings

## References

- [Discourse Modernization Guide](https://meta.discourse.org/t/modernizing-inline-script-tags-for-templates-js-api/366482)
- Repository: https://github.com/dereklputnam/discourse-netwrix-rebrand
